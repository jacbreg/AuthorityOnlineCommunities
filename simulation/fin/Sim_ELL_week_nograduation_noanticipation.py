...
stdata = states.copy()
stdata.loc[:,'isEditor'] = 0
stdata2 = states.copy()
stdata2.loc[:,'isEditor'] = 1
stdata = pd.concat([stdata,stdata2], axis=0, ignore_index=True)

statesreset = stdata.reset_index()
...
for period in range(max(periods)-1,0,-1):
    ...
    for choice in choices:
        ...
        tp = ssp.block_diag((tp,tp))
        EVF[:,choices.index(choice)] = delta * tp.dot(vf[period+1])[:,np.newaxis]
    U = U + EVF.toarray()
    vmax = np.max(U, axis=1)
    vf[period] = vmax
pd.to_pickle(vf, directory + 'vf_type{}_noantic.pkl'.format(typenum))
...
for period in range(num_periods):
    ...
    states = pd.merge(states, statesreset, on=state_names+['isEditor'], how='left')
...
simdata.to_csv(directory + 'simdata_type{}_noantic.csv'.format(typenum), index=False)
simdata.to_csv(basedir + 'DDCmodel\\Sim_types_nograd\\simdata_type{}_noantic.csv'.format(typenum), index=False)