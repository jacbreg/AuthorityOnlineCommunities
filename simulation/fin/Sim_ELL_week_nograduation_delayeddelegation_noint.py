...
def TPmatrix(choice):
    ...
    ssp.save_npz(directory2 + '%f_%f_%f_deldel.npz'%(choice[0],choice[1],choice[2]), matrix)
...
for period in range(max(periods)-1,0,-1):
    ...
    for choice in choices:
        tp = ssp.load_npz(directory2 + '%f_%f_%f_deldel'%choice + '.npz')
        EVF[:,choices.index(choice)] = delta * tp.dot(vf[period+1])[:,np.newaxis]
    U = U + EVF.toarray()
    vmax = np.max(U, axis=1)
    vf[period] = vmax
pd.to_pickle(vf, directory + 'vf_type{}_noint_deldel.pkl'.format(typenum))
...
simdata.to_csv(directory + 'simdata_type{}_noint_deldel.csv'.format(typenum), index=False)
simdata.to_csv(basedir + 'DDCmodel\\Sim_types_nograd\\simdata_type{}_noint_deldel.csv'.format(typenum), index=False)