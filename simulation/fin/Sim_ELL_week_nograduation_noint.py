...
for period in range(max(periods)-1,0,-1):
    ...
    vmax = np.max(U, axis=1)
    vf[period] = vmax
pd.to_pickle(vf, directory + 'vf_type{}_noint.pkl'.format(typenum))
...
simdata.to_csv(directory + 'simdata_type{}_noint.csv'.format(typenum), index=False)
simdata.to_csv(basedir + 'DDCmodel\\Sim_types_nograd\\simdata_type{}_noint.csv'.format(typenum), index=False)