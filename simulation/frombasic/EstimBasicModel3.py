...
'''
Estimation for simulated data created with SimBasicModel3.py
'''
...
directory = '/home/jacopo/OneDrive/Dati_Jac_locali/stack/apple/DDCmodel/SimBaseModel3_files/'

# import data
simdata = pd.read_csv(directory + 'simdataBasicModel3.csv', index_col=0)
...
for choice in choices:
    ...
    for period in range(1,rhoperiods+1):
        ...
        # MAIN
        mean_main = round(zeta*2*dff_main['expUVLAG%d'%(period-1)])/2 + c_main[period-1]
        ...
        # REF
        mean_ref = round(zeta*2*dff_ref['expUVLAG%d'%(period-1)])/2 + c_ref[period-1]
        ...
        for i in range(totextra_ref.shape[1]):
            futdata = np.array([totextra_ref[:,i].toarray().flatten(), dff_ref['expUVLAG%d'%(period)],dff_ref['avail%d'%(period)], dff_ref['period%d'%(period)]]).T
            p = clf.predict_log_proba(futdata)
            p = pd.DataFrame(p, columns=clf.classes_)
...
'''
      fun: 744.910670109964
 hess_inv: array([[ 7.97940865e-07, -5.01662441e-06],
       [-5.01662441e-06,  5.62051811e-03]])
      jac: array([0., 0.])
  message: 'Optimization terminated successfully.'
   ...
   status: 0
  success: True
        x: array([-0.01430977, -0.18394092])
'''

sopt.minimize(estim, x0=np.repeat(0,2), method='Nelder-Mead',options={'disp':True, 'maxiter':10000})    
'''
 final_simplex: (array([[-0.01430925, -0.18395806],
       [-0.01430969, -0.1838861 ],
       [-0.01430984, -0.18400069]]), array([744.91067028, 744.91067038, 744.91067044]))
           fun: 744.9106702836964
       message: 'Optimization terminated successfully.'
          nfev: 102
           nit: 54
        status: 0
       success: True
             x: array([-0.01430925, -0.18395806])
'''
hess_inv = np.array([[ 7.97940865e-07, -5.01662441e-06],
                     [-5.01662441e-06,  5.62051811e-03]])
diag_inv_hessian = np.diagonal(hess_inv)
print(np.sqrt(diag_inv_hessian))
'''[0.00089328 0.07497011]'''