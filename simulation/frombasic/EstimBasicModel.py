...
'''
Estimation for simulated data created with SimBasicModel.py
'''
...
directory = '/home/jacopo/OneDrive/Dati_Jac_locali/stack/apple/DDCmodel/SimBaseModel_files/'

# import data
simdata = pd.read_csv(directory + 'simdataBasicModel.csv', index_col=0)
...
for choice in choices:
    ...
    dff_out.loc[:,'A0'] = c_main[0] - c_ref[0]
    ...
    for period in range(1,rhoperiods+1):
        ...
        dff_out.loc[:,'A%d'%(period)] = c_main[period] - c_ref[period]
...
'''
      fun: 1346.286925956801
 hess_inv: array([[ 1.11888021e-05, -1.91778914e-04],
       [-1.91778914e-04,  5.03253413e-01]])
  ...
  message: 'Desired error not necessarily achieved due to precision loss.'
   ...
   status: 2
  success: False
        x: array([0.08898264, 0.01164617])
'''

sopt.minimize(estim, x0=np.repeat(0,2), method='Nelder-Mead',options={'disp':True, 'maxiter':10000})    
'''
 final_simplex: (array([[0.08898155, 0.0116356 ],
       [0.08898386, 0.01156563],
       [0.08898209, 0.01172474]]), array([1346.28692596, 1346.28692596, 1346.28692596]))
           fun: 1346.2869259587198
       message: 'Optimization terminated successfully.'
          nfev: 81
           nit: 43
        status: 0
       success: True
             x: array([0.08898155, 0.0116356 ])
'''
hess_inv = np.array([[ 1.11888021e-05, -1.91778914e-04],
                     [-1.91778914e-04,  5.03253413e-01]])
diag_inv_hessian = np.diagonal(hess_inv)
print(np.sqrt(diag_inv_hessian))
'''
approximative standard errors
[0.00334497 0.70940356]
'''