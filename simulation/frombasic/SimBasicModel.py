...
params = np.array([beta_rep,beta_effA])

### initial values for endogenous states (there's no exogenous states)
...
def computeFlow(st, ch, params):
    dta = np.array([st['rep_cum'], np.repeat(ch,len(st))]).T
...
for iteration in range(2): # average max utility across shocks
    v = []
    for c in choices:
        v.append(computeFlow(st_data, c, params) + np.random.gumbel())
    v = np.array(v)
...
for period in range(max(periods)-1,0,-1):
    ...
    for iteration in range(2):
        ...
        for c in choices:
            ...
            v.append(computeFlow(st_data, c, params) + delta * tp.dot(vfs[period+1]) + err)
        # same shock for all states, but choice specific
        v = np.array(v)
        vmax = np.max(v, axis=0)
        vfT.append(vmax)
...
pd.to_pickle(vfs, directory + 'vfs_simBase.pkl')
...
simdata.to_csv(directory + 'simdataBasicModel.csv')
