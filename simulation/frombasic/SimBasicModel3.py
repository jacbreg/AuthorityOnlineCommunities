...
for p in range(1,num_periods+1):
    max_possible_expuvlag = round(2* zeta * max_possible_expuvlag)/2 + max(choices)
...
for choice in choices:
    ...
    for row_num, state in st_data.iterrows():
        mean = round(2* zeta * state['expUVLAG'])/2 + choice
...
pd.to_pickle(vfs, directory + 'vfs_simBase3.pkl')
...
for period in range(num_periods):
    ...
    states.loc[:,'expUVLAG_t+1'] = round(zeta*2*states['expUVLAG'])/2 + states['choice']
...
simdata.to_csv(directory + 'simdataBasicModel3.csv')
